...
- **Collapsed pipeline view** — files grouped by AI-assigned classification into a horizontal left-to-right dependency flow. Expand one group at a time to drill in.
- **AI file classification** — LLM assigns each file a semantic role (`entry_point`, `api`, `business_logic`, `data_access`, `middleware`, `ui`, `utility`, `integration`, `background_jobs`, `config`, `test`).
- **AI summaries** — per-file and per-function natural language summaries generated by Llama 3 via Groq.
- **Semantic code search** — CodeBERT + MiniLM embeddings power intent-based queries ("where is auth handled?") with LLM-generated answers and Mermaid flow diagrams.
- **Priority ranking** — files scored by centrality, entry status, and cognitive load so you know where to start reading.
- **Ownership view** — contributor breakdown per file, commit counts, and ownership percentages.
- **Keyword references** — AI-extracted technical keywords linked to documentation and YouTube tutorials via Tavily.
- **Real-time progress** — live stage-by-stage analysis progress (clone → parse → graph → AI).
- **Multi-language support** — Python, TypeScript, JavaScript, Go, Rust, C/C++.
